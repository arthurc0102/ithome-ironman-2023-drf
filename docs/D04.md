...
再開始安裝之前我們先來說一下什麼是 DRF 跟為什麼我們要使用他。前面我們安裝的是 Django 他是一個 Python 的網頁框架，他包含了很多開發網頁會需要使用到的東西，例如 ORM 或是驗證系統等等（我們後續的文章會介紹到），他可以避免我們重複的造輪子。但他並沒有提供開發 API 所需要的功能，所以我們需要 DRF 他整合了前面提到 Django 的那些功能來協助我們快速的開發 API。所以我們可以把 DRF 想像成 Django 的擴充他讓我們可以在使用 Django 方便功能的同時也有方便的工具可以開發 API。
...
我們在上方的程式碼先把我們定義好的 view import 到 urls.py 這個檔案中，接著告訴 Django 說當使用者訪問 `/health`（註冊時開頭的 `/` 會審略）這個網址時執行我們的 HealthView。
